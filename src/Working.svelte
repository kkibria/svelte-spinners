<script>
  ...
  const margin = "1px";
	...
	let i = 1;
	colors.forEach(c => 
		ball_styles.push({c: c, d: `${i++ * 0.07}s`})
	);
</script>
...
<style>
	@keyframes sync {
		...
		33% {transform: translateY(calc(var(--size)*0.6));}
		66% {transform: translateY(calc(var(--size)*-0.6));}
...
<div class="wrapper" style="{css_var}">
	{#each ball_styles as s}
		<div class="balls" style="animation-delay: {s.d}; background-color: {s.c}"/>
	{/each}
</div>